...
class TestPositionSyncRefactored:
    ...
    def test_pnl_calculator_integration(self):
        ...
        realized_pnl = PnLCalculator.calculate_realized_pnl(
            opening_price=Decimal("2.50"),
            closing_price=Decimal("1.25"),
            quantity=1,
            is_credit=True,
    ...
    def test_combined_pnl_calculation_for_spread(self):
        ...
        short_pnl = PnLCalculator.calculate_leg_pnl(
            avg_price=5.50,
            current_price=3.25,
            quantity=1,
            quantity_direction="short",
        ...
        long_pnl = PnLCalculator.calculate_leg_pnl(
            avg_price=3.00,
            current_price=1.75,
            quantity=1,
            quantity_direction="long",
    ...
    def test_pnl_matches_tastytrade_formula(self):
        ...
        - Unrealized P&L = (Opening Credit - Current Mark) * Quantity * 100
        """
        # Real example: Opened spread for $2.80 credit, currently at $1.50
        opening_price = Decimal("2.80")
        current_mark = Decimal("1.50")
        quantity = 1

        our_pnl = PnLCalculator.calculate_unrealized_pnl(
            opening_price, current_mark, quantity, is_credit=True
        )

        # TastyTrade formula: (2.80 - 1.50) * 1 * 100 = $130
        expected_pnl = (opening_price - current_mark) * quantity * 100
...
class TestRealWorldScenarios:
    ...
    def test_senex_trident_pnl_calculation(self):
        """Test P&L for Senex Trident strategy (2 put spreads + 1 call spread)."""
        # Senex Trident:
        # - Put spread 1: Opened $2.50, now $1.50
        # - Put spread 2: Opened $2.50, now $1.50
        # - Call spread: Opened $0.75, now $0.50

        put1_pnl = PnLCalculator.calculate_unrealized_pnl(
            Decimal("2.50"), Decimal("1.50"), 1, is_credit=True
        )
        put2_pnl = PnLCalculator.calculate_unrealized_pnl(
            Decimal("2.50"), Decimal("1.50"), 1, is_credit=True
        )
        call_pnl = PnLCalculator.calculate_unrealized_pnl(
            Decimal("0.75"), Decimal("0.50"), 1, is_credit=True
        )
