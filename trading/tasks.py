...
def generate_and_email_daily_suggestions():
    ...
    Epic 22: Registry-based selection across all registered strategies.
...
async def _async_generate_and_email_daily_suggestions():
    ...
    for user in eligible_users:
        ...
        try:
            ...
            # Epic 22: Use StrategySelector directly (registry-based, no orchestrator needed)
...
async def _process_symbols_parallel(selector, symbols: list[str]) -> list[dict]:
    ...
    Args:
        selector: StrategySelector instance (Epic 22: registry-based)
...
@monitor_task
def reconcile_trades_with_tastytrade(self):
    """
...
async def _async_reconcile_trades():
    """Async implementation of trade reconciliation task."""
    from services.positions.lifecycle.trade_reconciliation_service import TradeReconciliationService

    logger.info("Starting trade reconciliation task.")
    ...
    logger.info(f"Reconciling trades for {len(users)} users with valid tokens")

    for user in users:
        try:
            logger.info(f"Reconciling trades for user {user.id}")
            service = TradeReconciliationService(user)

            # Standard reconciliation
            report = await service.reconcile_trades()

            if report.get("success"):
                summary["users_processed"] += 1
                summary["total_trades_updated"] += report.get("trades_updated", 0)
                summary["total_positions_updated"] += report.get("positions_updated", 0)

                if report.get("orphaned_orders"):
                    num_orphaned = len(report["orphaned_orders"])
                    logger.warning(f"User {user.id}: Found {num_orphaned} orphaned orders")
            else:
            ...
            try:
                ...
                if fix_report.get("positions_fixed", 0) > 0:
                    logger.info(
                        f"User {user.id}: Fixed {fix_report['positions_fixed']} stuck positions, "
                        f"created {fix_report['profit_targets_created']} profit target sets"
                    )
                    summary["total_positions_updated"] += fix_report["positions_fixed"]
                if fix_report.get("errors"):
                    logger.warning(
                        f"User {user.id}: Errors during stuck position fix: "
                        f"{fix_report['errors']}"
                    )
            except Exception as e:
                logger.error(f"User {user.id}: Error in fix_stuck_positions: {e}", exc_info=True)
                # Don't fail entire task - continue with next user

            # SELF-CORRECTION: Fix incomplete profit targets
            # Detects positions in open_full with missing profit targets and recreates them
            try:
                incomplete_report = await service.fix_incomplete_profit_targets()
                if incomplete_report.get("incomplete_targets_fixed", 0) > 0:
                    logger.info(
                        f"User {user.id}: Fixed {incomplete_report['incomplete_targets_fixed']} "
                        f"positions with incomplete profit targets"
                    )
                    summary["total_positions_updated"] += incomplete_report[
                        "incomplete_targets_fixed"
                    ]
                if incomplete_report.get("errors"):
                    logger.warning(
                        f"User {user.id}: Errors during incomplete target fix: "
                        f"{incomplete_report['errors']}"
                    )
            except Exception as e:
                logger.error(
                    f"User {user.id}: Error in fix_incomplete_profit_targets: {e}", exc_info=True
                )
                # Don't fail entire task - continue with next user

        except Exception as e:
            logger.error(f"Error reconciling user {user.id}: {e}", exc_info=True)
            summary["users_failed"] += 1
            summary["errors"].append(
                {
                    "user_id": user.id,
    ...
    logger.info(
        f"Reconciliation complete: {summary['users_processed']} users processed, "
        f"{summary['users_failed']} users failed, "
        f"{summary['total_trades_updated']} trades updated"
    )
...
def sync_order_history_task(self):
    ...
    try:
        ...
        if self.request.retries < self.max_retries:
            countdown = 60 * (2**self.request.retries)
            logger.info(f"Retrying sync_order_history_task in {countdown}s")
            raise self.retry(countdown=countdown)
        return {
            "status": "failed",
...
async def _async_sync_order_history():
    """Async implementation of order history sync task."""
    from services.orders.history import OrderHistoryService

    logger.info("Starting order history sync task")
    ...
    for account in accounts:
        try:
            logger.info(f"Syncing order history for account {account.account_number}")
            result = await service.sync_order_history(account, days_back=7)

            if result.get("errors"):
                logger.warning(
                    f"Order history sync for {account.account_number} "
                    f"had {len(result['errors'])} errors"
                )

            summary["accounts_processed"] += 1
            summary["total_orders_synced"] += result.get("orders_synced", 0)
            summary["total_new_orders"] += result.get("new_orders", 0)

            logger.info(
                f"Synced {result['orders_synced']} orders for account "
                f"{account.account_number} ({result['new_orders']} new)"
            )

        except Exception as e:
            logger.error(
                f"Error syncing order history for account " f"{account.account_number}: {e}",
                exc_info=True,
            )
    ...
    logger.info(
        f"Order history sync complete: {summary['accounts_processed']} accounts processed, "
        f"{summary['accounts_failed']} accounts failed, "
        f"{summary['total_orders_synced']} orders synced "
        f"({summary['total_new_orders']} new)"
    )
...
def sync_positions_task(self):
    ...
    try:
        ...
        if self.request.retries < self.max_retries:
            countdown = 60 * (2**self.request.retries)
            logger.info(f"Retrying sync_positions_task in {countdown}s")
            raise self.retry(countdown=countdown)
        return {
            "status": "failed",
            "users_processed": 0,
...
async def _async_sync_positions():
    """Async implementation of position sync task."""
    from services.positions.sync import PositionSyncService

    logger.info("Starting position sync task")
    ...
    # Get all active users with primary trading accounts that have valid tokens
    users = await sync_to_async(list)(
        User.objects.filter(
            is_active=True,
            trading_accounts__is_primary=True,
    ...
    logger.info(f"Syncing positions for {len(users)} active users with valid tokens")

    service = PositionSyncService()

    for user in users:
        try:
            logger.info(f"Syncing positions for user {user.id}")
            result = await service.sync_all_positions(user)

            if result.get("error"):
                logger.error(f"Position sync for user {user.id} failed: {result['error']}")
                summary["users_failed"] += 1
                summary["errors"].append(
                    {
                        "user_id": user.id,
                ...
                summary["users_processed"] += 1
                summary["total_positions_synced"] += result.get("positions_found", 0)
                summary["total_positions_updated"] += result.get("updated", 0)

                logger.info(
                    f"Synced {result.get('positions_found', 0)} positions for user {user.id} "
    ...
    logger.info(
        f"Position sync complete: {summary['users_processed']} users processed, "
        f"{summary['users_failed']} users failed, "
        f"{summary['total_positions_synced']} positions found"
    )
...
def persist_greeks_from_cache(self):
    ...
    Epic 05, Task 004: Greeks Historical Persistence
